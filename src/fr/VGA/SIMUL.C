   ...
   if (!Hole_DMuLNG())
    {
       ...
       if (!trace) {
		     Meldung("Frapper une touche pour commencer.");
		     Status("J'attends");
		     while (kbhit());
		     while (!kbhit());
	  ...
	  for (ip=0;!prg_ende;ip++)
		...
		Zeile2SCR(ip);
		if (trace) {
			     Meldung("Frappez touche pour continuer...");
			     Status("Trace-Stop");
			     while (kbhit()) getch();
			     while (!kbhit());
			     prg_ende=Check_FlagTasten();
       ...
       if (sp!=0 && (!esc || !ok))
	{
	  Meldung("M30 trouv‚ dans un sous-programme!");
	  Status("Erreur!");
	}
       ...
       if (esc)
	{
	  Meldung("Interruption avec ESC - Touche pour terminer.");
	  Status("Interruption.");
	}
...
char Execute_M(char *ok,char *esc)
{
  ...
  Status("Simulation");
  Meldung("Traitement d'une commande M");
  ...
  switch(m_wert)
  {
     /**************** M00 *******************/
     case 0:{
	      Meldung("Frappez une touche pour continuer");
	      getch();
	      Meldung(" ");
	    } break; /* No Action */
     ...
     /**************** M02: Pseudo: Stueck neuzeichnen *******************/
     case 2:Zeichne_stueck();
	    break;
     /**************** M03 *******************/
     ...
     case 17:if (sp>0)
	     ...
	     {
	       Meldung("Erreur: M17 n'est pas dans un sous-programme!");
	       Status("Erreur");
	       endprg=1; *ok=0;
	     }break;
     /**************** M30 *******************/
     ...
     case 99:{
	       ...
	       Meldung("Commande M99 seule trouv‚e!!");
	       Status("Erreur!");
	       endprg=1;
	       *ok=0;
	     }
  ...
  if (!*ok && endprg)
	    {
	      Status("E-Interruption");
	      Meldung("Commande M dans la ligne courante erron‚e ou inconnue!");
	    }
...
char Execute_G(char *ok,char *esc)
{
  ...
  switch(g_wert)
  {
     ...
     case 02:if (cncprg[ip].XI[0]!='I' && cncprg[ip].ZK[0]!='K')
		...
		if (!strcmp(cncprg[ip+1].GM,"M99 "))
			...
			{
			  Meldung("Erreur dans la commande M99!");
			  Status("Erreur!");
			  getch();
			  endprg=1; *ok=0;
			} else
     ...
     case 25: /* Unterprgr */
	     ...
	     {
		Status("Erreur!");
		Meldung("Erreur dans la commande GOSUB G27!");
		*ok=0; endprg=1;
	     }break;
     /**************** G26 *******************/
  ...
  if (!*ok && endprg)
	    {
	      Status("E-Interruption");
	      Meldung("Commande G dans la ligne courante erron‚e ou inconnue!");
	    }
...
char Check_FlagTasten(void)
{
  ...
  if (kbhit())
	...
	case 's':{
		   Meldung("Continuer avec 'A' - Interruption avec ESC");
		   Status("Pause");
		   do {
			taste=(char)getch();
...
char Ziehe_KreisNO90(short xr1,short zp1,short xr2,short zp2,short xrm,short zpm,char g03g02)
{
  ...
  if (quadr1!=quadr2) { /* Routine zZ noch nicht in Funktion */
			Meldung("M99: Erreur de quadrant!");
			Status("Erreur!");
			getch();
			endprg=1;
		      }
  ...
  if (fabs(fabs(rad1)-fabs(rad2))>160)
    {
      Meldung("M99: Tolerance de 0,8mm d‚pass‚e!");
      Status("Erreur");
      getch();
      endprg=1;
    }
...
double arc_wi(double sinw,double cosw,char *quadrant)
{
  ...
  if (sinw!=0.0 && cosw!=0.0)
	...
	if (quadr==1 || quadr==2) endw=(cosw1<cosw2) ? cosw1:cosw2;
     ...
     else  /* Wenn Punkt aus Achse -> Sonderbehandlung */
	   {
	     ...
	     /* Winkel wieder in [0-360ø] zwingen */
  ...
  /* Quadrant des Endwinkels feststellen.
     ...
     behandlung (0,90,180,270ø) der Quadrant nicht vorberechnet wird */
...
void Automatisch_Ankratzen(void)
{
   ...
   Meldung("Charriage automatique de la piŠce!");
   Status("Point d'origine");
...
char Manuell_Nullpunkt_festlegen(void)
{
   ...
   Meldung("Choisissez le point de d‚part s.v.p.: (Crtl+) Curseur & ENTER");
   Status("point de d‚part");
   zpos=100;
   xrad=radius+100;
...
void SimBildschirmmaske(void)
{
  ...
  _outtext(" Simulateur: ³ Fonct. de pause:");
  ...
  _outtext("X-diamí:                     Outil: T01 (Ref)        Coordonn‚es:INC \n");
  _outtext("Z-pos  :                     Trace: Off \n");
  _outtext("\nPiŠce    : l=");
  _outtext(buf_l);
  _outtext("mm  í=");
  _outtext(buf_d);
...
char Hole_DMuLNG(void)
{
  ...
  _outtext("Simulation:\nÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ\nEntrez les donn‚es de la piŠce s.v.p.\n\n");
  _outtext("DiamŠtre    (1/100 mm : 100 =1 mm):");
	   ...
	   return 1;
		...
		if (laenge<1000 || laenge>17000 || durchm>8000 || durchm<300 ||
		    idurchm>durchm-700 || (idurchm<1000 && idurchm!=0) || idurchm>15000)
		    {
		      ...
		      _outtext("          ***** ATTENTION *****\n");
		      _outtext(" Limites   : DiamŠtre   : 300 bis 8000\n");
		      _outtext("                         (3 mm - 80 mm)\n");
		      _outtext("             Longueur   : 1000 bis 17000\n");
		      _outtext("                         (10 mm - 170 mm)\n");
		      _outtext("             í int‚rieur: 0 / 1000 bis 15000\n");
		      getch();
		      TW_Close_Window();
		      _settextcolor(10);
		      _setbkcolor(4);
...
void Werkzeuge_darstellen(void)
{
   ...
   _outtext("Outil … dresser");
   _settextposition(9,26);
   _outtext("… gauche");
   _settextcolor(13);
   ...
   _outtext("Outil tourage");
   _settextposition(9,46);
   _outtext("de finition");
   _settextcolor(13);
   ...
   _outtext("T04");
   _settextposition(8,64);
   _outtext("Outil … fileter");
   _settextposition(9,63);
   _outtext("ext‚rieur … droite");
   _settextcolor(13);
   ...
   _outtext("  X=-10mm Z=0\n");
   _settextcolor(12);
   _outtext("largeur   :1,2mm\n");
   _outtext("profondeur:1,5mm\n");
   ...
   _outtext("T07");
   _settextposition(21,41);
   _outtext("Outil … filiter");
   _settextposition(22,40);
   _outtext("int‚rieur droite");
   _settextcolor(13);
   _settextposition(23,43);
   _outtext("Correction:");
