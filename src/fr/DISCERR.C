...
char DE_FileExit(char *name)
{
   ...
   char pdarray[2][16]={"    Effacer    ","  Nouveau nom  "},
	pdrp   [2]    ={1,1};
   pd.array=pdarray[0];
   pd.relpos=pdrp;
   ...
   _settextcolor(7);
   _setbkcolor(1);
   TW_Open_Window(4,5,33,4,TW_RAHMEN_S1,TW_TITEL_OBEN,3,"AVERTISSEMENT");
   _outtext("* Le fichier actuel existe d‚j… *");
   pd_ret=PD_Menue(&pd);
   switch(pd_ret)
   {
...
char DE_DiskError(FILE *dateizgr,char *dateiname)
{
    ...
    /* PD-Menue-Var */
    char relpos[2]={1,1};
    char array[2][15]={" Interruption ","  R‚p‚tition  "};
    struct PD_parameter RetCan={27,7,0,1,1,0,0,2, 15,NULL,NULL};
    char wahl,ret;
	...
	if (ax==0) /* Prioritaet auf AX/DI-Fehler */
	    { /* Fehler beim Oeffnen */
	      strcpy(errstr1,"Fichier:");
	      strcat(errstr1,dateiname);
	      strcpy(errstr2,"Impossible d'ouvrir le fichier!");
	      strcpy(errstr3,"* Verifiez nom et chemin S.V.P.! *");
	    }
	   else {
...
void GeneriereErrorStr(unsigned ax,unsigned di,char *es1,char *es2,char *es3)
{
   ...
   {
      case 0x00:{
		  strcpy(es2,"Protection d'‚criture est mise!");
		  strcpy(es3,"Enlevez la protection s.v.p.");
		}
		break;
      case 0x01:{
		  strcpy(es2,"Faute sp‚cification du lecteur!");
		  strcpy(es3,"Unit‚s possibles: ");
		...
		break;
      case 0x02:{
		  strcpy(es2,"Lecteur n'est pas prŠs!");
		  strcpy(es3,"Verifiez s'il y a une disquette");
		}
		break;
      case 0x03:{
		  strcpy(es2,"Invalid Command!");
		  strcpy(es3,"!! Erreur systŠme !!");
		}
		break;
      case 0x04:{
      ...
      case 0x05:{
		...
		break;
      case 0x06:{
	       strcpy(es2,"Erreur de lecture ou ‚criture.");
	       strcpy(es3,"(Une faute sur la disquette.)");
	     }
	     break;
      case 0x07:{
	       strcpy(es2,"Format de la disquette inconnu.");
	       strcpy(es3,"Utilisez une autre disquette s.v.p.");
	     }
	     break;
      case 0x08:{
		  strcpy(es2,"Secteur introuvable!");
		  strcpy(es3,"Erreur sur la disquette.");
		}
		break;
      case 0x09:{
		  strcpy(es2,"Printer out of paper!");
		  strcpy(es3,"Erreur d'imprimante");
		}
		break;
      case 0x0A:{
		  strcpy(es2,"Erreur d'‚criture!");
		  strcpy(es3,"Faute cluster/secteur d‚tect‚e");
		}
		break;
      case 0x0B:{
		  strcpy(es2,"Erreur de lecture!");
		  strcpy(es3,"Faute cluster/secteur d‚tect‚e");
		}
		break;
      case 0x0C:{
		  strcpy(es2,"Erreur g‚n‚rale!");
		  strcpy(es3,"(Avez-vous utiliser une disquette format‚e?)");
		}
		break;
      default  :{
		  strcpy(es2,"!! Erreur de disquette inconnue !!");
		  strcpy(es3,"");
		}
		break;
   }
