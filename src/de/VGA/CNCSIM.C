...
/*
  CNC-Programm-Simulator V1.2 BETA VGA DT fuer EMCO-Drehbaenke
  Programm   : Tasha Carl, http://carl.pro
  Compiler   : MS C6 Model MEDIUM (DATA NEAR/CODE FAR)
  Date       : 04/1992
  Version    : VGA (nur CNCSIM.C & SIMUL.C) Deutsch
*/
...
void ProgrammInit(void)
{
  ...
  /* TimeOut-Var setzten : Verminderung des Hand-Snake-Effekts wenn vorhanden */
  to1=*to_pt;
  to2=*(to_pt+2);
  to3=*(to_pt+4);
  to4=*(to_pt+8);
...
void Bildschirmmaske(void)
{
   ...
   _outtext("    EMCO-CNC-Simulator V1.2 VGA DT BETA              STS-CNC-Projekt 4.1992     ");
   SYS_PopColor();
   ...
   if (MON2_Init())
	 ...
	 MON2_outtext("EMCO-CNC-Simulator");
	 MON2_settextposition(1,4);
	 MON2_outtext("Programmiert von Tasha Carl (Made in Belgium/Eupen)");
       }
}
...
void Hauptmenue(char *w1,char *w2)
{
   ...
   static struct PD_parameter uebertr={3,4,0,1,0, 0,1, 6, 24,NULL,NULL},
   ...
   static char balken[100]={" HauptmenÅ: | öbertragen |  | Editor |  | Simulation |  | Optionen |  | Quit |     | Info | "};
   /* Kombiniertes Menue */
