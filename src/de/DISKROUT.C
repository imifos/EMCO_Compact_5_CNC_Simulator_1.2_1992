...
void Disc_Mem(void)
{
   ...
   _settextcolor(3);
   _setbkcolor(9);
   TW_Open_Window(15,6,65,12,TW_RAHMEN_S1,TW_TITEL_OBEN,5,"Programm laden...");

   while (ok && !laden)
     {
	_clearscreen(_GWINDOW);
	_outtext("\nBitte wÑhlen Sie das Programm:");
	_outtext("\n(Mit RETURN werden die Infos angezeigt)");
	...
	if (!prg_gesichert)
	    {
	       _settextcolor(2);
	       _outtext("\n\a*** ACHTUNG ! Aktuelles Programm nicht gesichert ! ***");
	       _settextcolor(3);
	    }
	...
	if ((ok=DIR_Wahl(22,13,"*.cnc", name,pfad,&laufw))==1)
	  ...
	  else _outtext("\n\n\n\nAbbruch im Directory!");

     } /* von while ok & !laden */

   if (!ok)
     { _outtext("\nLaden abgebrochen!"); getch(); }

   if (laden)
     {
       _clearscreen(_GWINDOW);
       ErrorInit(); /*Errorfeld init*/
       _outtext("Lade Programm:\n");
       _outtext(bprgname);
...
void Mem_Disc(void)
{
   ...
   TW_Open_Window(2,3,78,21,TW_RAHMEN_S1,TW_TITEL_OBEN,5,"Programm sichern...");
   ...
   if (!esc)
    {
       ...
       if (flag==DE_NOERROR)
	 {
	    ...
	    for (za=0;za<max_line;za++)
		{
		  ...
		  if (write_anz==0) {
				      _outtext("Kein Diskettenspeicher mehr - ");
				      flag=DE_ERROR; break;
				    }
		  if (kbhit()) if (getch()==27) {
						  _outtext("Abbruch durch ESC -");
						  flag=DE_ERROR; break;
						}
		}
	    fcloseall();
	    if (flag==DE_ERROR)
		{ _outtext("Fehler beim Schreiben!"); printf("\a"); }
		else {
		       _outtext("Programm erfolgreich gesichert!");
		       prg_gesichert=1;
		     }
	    getch();
	 }
...
char Syntax(void)
{
   ...
   _settextcolor(7);
   _setbkcolor(1);
   TW_Open_Window(15,10,30,4,TW_RAHMEN_D2,TW_TITEL_OBEN,3,"Programmsyntax");
   ...
   for (zeile=0;zeile<bis_zeile && !esc;zeile++)
      ...
      if (Test_Zeile(zeile))
	{
	  ...
	  _outtext("Letzter Fehler in Zeile ");
	  sprintf(buf,"%03d ",zeile);
	  _outtext(buf);
	  error=1;
	}
   ...
   if (error) {
		...
		_outtext("Fehler im Programm gefunden!");
		getch();
	      }
...
void EMCO_Mem(void)
{
   ...
   if (!SYS_ExistCOM(prgconfig.com))
    {
      ...
      _settextcolor(3);
      _setbkcolor(9);
      TW_Open_Window(25,10,51,3,TW_RAHMEN_S1,TW_TITEL_OBEN,5,"Fehler");
      _outtext(" Fehler:\n Configurierte COM-Schnittstelle nicht vorhanden!\n (Taste)");
      getch();
      TW_Close_Window();
      SYS_PopColor();
      return;
   ...
   /* Maske */
   _outtext("EMCO-Programm wird Åber die serielle Schnitt-stelle eingeladen:");
   _outtext("\n\nStatus:\nLinie :ID\n\n\n          (DrÅcke ESC zum Abbruch)");
   ...
   _outtext("Warte auf Programmdaten...");
   do {
   ...
   if (esc!=27)
    {
      ...
      _settextposition(4,8);
      _outtext("Empfange Programmdaten...");
      _settextcolor(3);
      punktzaehler=0;
      comza=0;
      oldwert=0;
   ...
   if (esc==27) _outtext("    Abbruch durch ESC!    ");
      else _outtext("   öbertragung beendet!  ");
   getch();
   SYS_PopColor();
   TW_Close_Window();
}
...
void Mem_EMCO(void)
{
   ...
   if (!SYS_ExistCOM(prgconfig.com))
    {
      ...
      _settextcolor(3);
      _setbkcolor(9);
      TW_Open_Window(25,10,49,3,TW_RAHMEN_S1,TW_TITEL_OBEN,5,"Fehler");
      _outtext(" Fehler:\n Configurierte COM-Schnittstelle nicht vorhanden!\n (Taste)");
      getch();
      TW_Close_Window();
      SYS_PopColor();
   ...
   /* Maske */
   sprintf(buf,"Linien:%d (pro Linie ca. 1 Sek.)",bis_zeile-1);
   _outtext("CNC-Programm wird Åber  die serielle Schnitt-stelle zur EMCO-Maschine geschickt:\n");
   _outtext(buf);
   _outtext("\nLinie :ID");
   ...
   if (Syntax()) {
		   ...
		   _outtext("Programm enthÑlt Fehler!");
		   _settextposition(7,6);
		   _outtext("Trotzdem öbertragen?         ");
   ...
   if (weiter)
    {
      ...
      _outtext("     Bitte eine Taste drÅcken zum Beginnen  ");
      esc=getch();
      _settextposition(7,1);
      _outtext("           (DrÅcke ESC zum Abbruch)         ");
   ...
   if (esc==Sescape && esc!=Serror)
		_outtext("                   Abbruch!                ");
   if (esc!=Sescape && esc!=Serror)
		_outtext("             öbertragung beendet!          ");
   if (esc!=Sescape && esc==Serror)
		_outtext("      Fehler beim öbertragungsversuch!     ");
   getch();
   SYS_PopColor();
   TW_Close_Window();
}
