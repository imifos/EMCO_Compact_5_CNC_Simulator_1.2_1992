...
#define schritt_g01 25	  /* Abnehmen */
...
{
   short yp_p=Unit2PntY(xr_mm),
	 xp_p=Unit2PntX(zp_mm),
	 dx=xp_p,
	 dy=yp_p;
...
short Unit2PntY(short mm_div_100)
{
   short ret=(short)( ((float)mm_div_100/100.0F)*2.0F );
   return ret;
}

short Unit2PntX(short mm_div_100)
{
   short ret=(short)( ((float)mm_div_100/100.0F)*4.0F );
   return ret;
}
	   ...
	   return 1;
		...
		if (laenge<1000 || laenge>15000 || durchm>8000 || durchm<300 ||
		    idurchm>durchm-700 || (idurchm<1000 && idurchm!=0) || idurchm>10000)
		    {
		      ...
		      _outtext("                         (3 mm - 50 mm)\n");
		      _outtext("             LÑnge      : 1000 bis 15000\n");
		      _outtext("                         (10 mm - 150 mm)\n");
		      _outtext("             InnenÌ     : 0 / 1000 bis 10000\n");
...
{
    short xp=Unit2PntX(zpos_mm),
	  yp=Unit2PntY(xrad_mm);
...
/* 2. Hercules-Monitor initialisieren */
void Init2HGCMon(void)
{
  MON2_clrscr(1,1,80,25);
  MON2_settextposition(1,1);
  MON2_outtext("EMCO-CNC-Simulator : UnterstÅtzung in einer zukÅnftigen Version vorgesehen.");
}
...
/* Werkzeugbildschirm : wird vom Editor eingeblendet */
/****************************************************************
WERKZEUGBILDSCHIRM
*****************************************************************/
