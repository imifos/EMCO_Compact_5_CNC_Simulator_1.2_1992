...
void LoadConfiguration(struct configstr *config)
{
  ...
  do {
       ...
       error=(char)DE_DiskError(configdatei,"Conifurationsdatei");
  ...
  if (error==DE_NOERROR)
    {
      ...
      if (anz==0 || strcmp(config->intern1,"ëeeN⁄,d≤")) error=DE_ERROR;
    } /* von Laden */
  ...
  if (error==DE_ERROR)
    {
      ...
      TW_Open_Window(10,10,51,4,TW_RAHMEN_D1,TW_TITEL_UNTEN,2,
		    "Configuration");
      _settextcolor(0);
      _outtext(" *** Fehler beim Laden der Configurationsdatei ***\n");
      _settextcolor(3);
      _outtext(" Erzeuge neue Configurationsdatei:\n");
      ...
      strcpy(config->intern1,"ëeeN⁄,d≤");
      strcpy(config->intern2,"-NY7Ò");
      config->monitor2=0;
      config->com=0;
	 ...
	 printf("Error beim Erzeugen einer neuen CONFIGDATEI!");
	 printf("CNCSIM Ende!");
	 exit(1);
       }
...
void Optionen(char ywahl)
{
   ...
   switch(ywahl)
   {
     ...
     case 2:{ /* 2. HGC MON - Nachricht anzeigen */
	      ...
	      _settextcolor(3);
	      _setbkcolor(12);
	      TW_Open_Window(5,17,67,1,TW_RAHMEN_D1,TW_TITEL_UNTEN,2,"2. BILDSCHIRM");
	      _settextcolor(0);

	      if (VGA)
		if (MON2_vorhanden)
		    _outtext(" Es wurde eine 2. Grafikkarte im System gefunden!");
		    else _outtext(" Es wurde keine 2. Grafikkarte im System gefunden!");
...
void H_Wert_eingeben(void)
{
   ...
   _settextcolor(3);
   _setbkcolor(12);
   TW_Open_Window(5,17,70,2,TW_RAHMEN_D1,TW_TITEL_UNTEN,2,"Standart-H-Wert:");
   _settextcolor(0);

   sprintf(buffer,"%-4d",prgconfig.h_norm);
   _outtext("Wird im Programm ein H-Wert nicht eingegeben, wird dieser Wert eingesetzt:");
   _outtext("Standart-Wert:");
...
void Wartezeiten(void)
{
   ...
   _settextcolor(3);
   _setbkcolor(12);
   TW_Open_Window(10,17,59,4,TW_RAHMEN_D1,TW_TITEL_UNTEN,2,"Simulationswartezeiten");
   _settextcolor(0);

   sprintf(g00,"%-4d",prgconfig.wait_g00);
   sprintf(g01,"%-4d",prgconfig.wait_g01);

   _outtext("Wartezeiten zwischen den Bewegungsphasen in der Simulation:");
   _outtext("(Gibt die Anzahl der Warteschleifen an)\n");
   _outtext("Eilgang        (G00)      :");
   _outtext(g00);
   _outtext("\nSpanungszyklen (G01/02/..):");
   _outtext(g01);
   SYS_CursorOn();
...
void COM_waehlen(void)
{
   ...
   _settextcolor(3);
   _setbkcolor(12);
   TW_Open_Window(10,10,47,10,TW_RAHMEN_D1,TW_TITEL_OBEN,2,"Serielle Schnittstelle");
   _settextcolor(0);

   _outtext(" Bitte wÑhlen Sie ihre serielle Schnittstelle:\n\n\n\n\n\n\n\n");
   _outtext("  Die  mit  *  bezeichneten Adressen sind bei\n");
   _outtext("      dieser Maschine nicht verfÅgbar!");
...
void LPT_waehlen(void)
{
   ...
   _settextcolor(3);
   _setbkcolor(12);
   TW_Open_Window(10,10,45,10,TW_RAHMEN_D1,TW_TITEL_OBEN,2,"Druckerschnittstelle");
   _settextcolor(0);

   _outtext(" Bitte wÑhlen Sie ihre Druckerschnittstelle:\n\n\n\n\n\n\n\n");
   _outtext(" Die  mit  *  bezeichneten Adressen sind bei\n");
   _outtext("     dieser Maschine nicht verfÅgbar!");
...
void Menue_definieren(char flag)
{
  ...
  static char array1[5][12]={"   LPT 1   ","   LPT 2   ","   LPT 3   ","   LPT 4   ","   Ende    "};
  static char array2[5][12]={"   COM 1   ","   COM 2   ","   COM 3   ","   COM 4   ","   Ende    "};
